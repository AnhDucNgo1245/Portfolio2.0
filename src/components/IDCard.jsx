...
export default function IDCard() {
  ...
  const gloss = useSpring(useTransform(x, [-100, 100], [0.3, 0.6]), springConfig)
  ...
  return (
    <div className="flex flex-col items-center select-none">
      ...
      <svg width="60" height="60" viewBox="0 0 60 60" className="mb-0">
        <path d="M30 0 Q20 20 15 40 Q20 50 30 60 Q40 50 45 40 Q40 20 30 0Z"
          fill="url(#ribbon)" opacity="0.9" />
        <defs>
      ...
      <motion.div
        ...
        className="relative w-64 rounded-2xl overflow-hidden shadow-2xl shadow-cyan-900/40 border border-cyan-500/30"
      >
        {/* Card background */}
        ...
        <motion.div
          ...
          style={{
            background: useTransform(gloss, v => 
        ...
        <div className="relative px-6 py-5 flex flex-col items-center">
          {/* Logo/name bar */}
          <div className="w-full mb-4 flex items-center justify-between text-[10px] uppercase tracking-[0.3em] text-cyan-500/60 border-b border-cyan-500/20 pb-2">
            <span>NDA</span>
            <span className="font-serif text-lg text-cyan-400/30">劍</span>
            <span>2025</span>
          </div>

          {/* Avatar */}
          <div className="relative mb-4">
            <div className="absolute -inset-1 rounded-full bg-gradient-to-br from-cyan-400 to-emerald-400 blur-sm opacity-50" />
            <img
              src={avatarImg}
              alt="Avatar"
          ...
          {/* Name */}
          <h2 className="text-white font-black text-xl tracking-wider uppercase mb-1">
            NGÔ ĐỨC ANH
          </h2>
          ...
          <div className="w-full flex items-center justify-between">
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, i) => (
                <div key={i} className={`w-1 bg-cyan-500/30 rounded-sm`} style={{ height: `${8 + i * 3}px` }} />
              ))}
            </div>
            <p className="text-[9px] text-slate-600 tracking-widest">CYBER-SWORDCRAFT</p>
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, i) => (
                <div key={i} className={`w-1 bg-emerald-500/30 rounded-sm`} style={{ height: `${20 - i * 3}px` }} />
              ))}
            </div>
          </div>
        </div>
